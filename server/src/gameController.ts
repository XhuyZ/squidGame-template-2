...
export class GameController {
    ...
    private gameState: GameState = {
        gameName: 'lobby',
        players: [],
        adminStats: { // Initialize adminStats
            totalPlayers: 0,
            alivePlayers: 0,
            eliminatedPlayers: 0,
            game1Scores: [],
    ...
    private nextQuestion(): void {
        ...
        const activePlayers = this.gameState.players.filter(p => p.status === 'alive' && !p.isAdmin);
    ...
    private endCurrentGame(): void {
        ...
        const aliveCount = this.gameState.players.filter(p => p.status === 'alive' && !p.isAdmin).length;
        if (aliveCount <= 1) {
            this.endGame();
            return;
        }
    ...
    private endGame(): void {
        ...
        const potentialWinners = this.gameState.players.filter(p => p.status === 'alive' && !p.isAdmin);
        if (potentialWinners.length > 0) {
            this.gameState.winner = [...potentialWinners].sort((a,b) => b.score - a.score)[0];
        } else if (this.gameState.players.length > 0) {
            // If no alive non-admins, pick highest score from all non-admins
            this.gameState.winner = [...this.gameState.players.filter(p => !p.isAdmin)].sort((a,b) => b.score - a.score)[0];
        }
        this.broadcastSound('cheer');
        this.broadcastGameState();
        this.io.emit('notification', `${this.gameState.winner?.name || 'No one'} wins the Squid Game!`);
    }
    ...
    public handleAnswer(id: string, answer: string): void {
        ...
        if (!player || player.status === 'out' || player.answered || !question || player.isAdmin) return; // Admins don't answer
    ...
    private handleTimeUp(): void {
        this.gameState.players.forEach(p => {
            if (p.status === 'alive' && !p.answered && !p.isAdmin) { // Only affect non-admin active players
                p.hp -= 1;
                if (p.hp <= 0) {
                    p.status = 'out';
                    this.io.emit('notification', `${p.name} has been eliminated (time up)!`);
                }
            }
        });
    ...
    private checkRoundCompletion(): void {
        ...
        if (allAnswered || activePlayers.length === 0) { // If all active players answered or no active players left
    ...
    private checkGameStatus(): void {
        if(this.gameState.gameName !== 'lobby' && this.gameState.gameName !== 'winner') {
            const aliveCount = this.gameState.players.filter(p => p.status === 'alive' && !p.isAdmin).length; // Only non-admin players
            if (aliveCount <= 1) {
                this.endCurrentGame();
            }
        }
    ...
    private resolveTugOfWarRound(): void {
        if (!this.gameState.teams || !this.gameState.tugOfWar || !this.gameState.currentQuestion) return;

        const redCorrect = this.gameState.teams.red.filter(p => p.answered && p.status === 'alive' && p.id !== undefined && this.gameState.currentQuestion?.answer === questionsGame2[this.gameState.round!].answer).length;
        const blueCorrect = this.gameState.teams.blue.filter(p => p.answered && p.status === 'alive' && p.id !== undefined && this.gameState.currentQuestion?.answer === questionsGame2[this.gameState.round!].answer).length;
    ...
    private calculateScores(): void {
        ...
        const currentAlivePlayers = this.gameState.players.filter(p => p.status === 'alive' && !p.isAdmin);
        ...
        if (this.gameState.gameName === 'game1') {
            ...
            currentAlivePlayers.forEach((p, index) => {
                ...
                p.score += points;
                this.gameState.adminStats.game1Scores.push({ name: p.name, score: points });
            });
            this.gameState.adminStats.game1Scores.sort((a, b) => b.score - a.score); // Sort for display
        } else if (this.gameState.gameName === 'game2') {
            if ((this.gameState.tugOfWar?.position ?? 0) > 0) { // blue won
                this.gameState.teams?.blue.forEach(p => {
                    if (p.status === 'alive' && !p.isAdmin) {
                        p.score += 10;
                        this.gameState.adminStats.game2Scores.push({ name: p.name, score: 10 });
                    }
                });
                this.io.emit('notification', 'Blue Team wins Tug of War!');
            } else if ((this.gameState.tugOfWar?.position ?? 0) < 0) { // red won
            ...
            currentAlivePlayers.forEach(p => {
                ...
                p.score += points;
                this.gameState.adminStats.game3Scores.push({ name: p.name, score: points });
            });
    ...
    private updateAdminStats(): void {
        ...
        this.gameState.adminStats = {
            ...
            eliminatedPlayers: nonAdminPlayers.filter(p => p.status === 'out').length,
    ...
    this.broadcastGameState();
    this.io.emit('notification', 'Game has been reset. Waiting for players to join.');
}
}